def D_score(df, param, name):
    ...
    #outliers detection
    #Smoothed_ACCECPTEDDATA: This variable are the smoothed data provinding of the kernel smoother process. It's a pre-treatment.   
    #param: This variable corresponds chosen parameters by the operator. Then definition of this one are in the function DefaultParam.
    ...
    #Q_slope: Slopes calculated between two smoothed data. Gives information about the dynamics of the data and helps detection of too sudden. 
...
def single_sample_runs_test(AcceptedData,SMOOTHED_ACCEPTEDDATA,nb_data,param):
    ...
    for i in range(h_smoother, len(AcceptedData)-h_smoother):#i=h_smoother+1:length(AcceptedData)-h_smoother #start at h+1
        
        t[i]=np.sign(AcceptedData[i]-SMOOTHED_ACCEPTEDDATA[i]) #Determination of the residual sign between the smoothed data and the AccepetedData
        run[i]=abs(t[i]-t[i-1])/2  #Determination of where there is a sign change in the data serie
...
def Quality_D(AcceptedData, SMOOTHED_ACCEPTEDDATA,nb_data,param):
    ...
    #The INPUT of this function are: 
    #SMOOTHED_ACCEPTEDDATA:This variable are the smoothed data provinding of the kernel smoother process. It's a pre-treatment. 
    #transAcceptedData/transSMOOTHED_ACCEPTEDDATA:Logarithmic transformation to stabilize the residual standard deviation, if it is necessary
    #nb_data: This varaible represents the data number of the sample.
    ...
    Q_slope=np.zeros((len(SMOOTHED_ACCEPTEDDATA),))#Slopes calculated between two smoothed data
